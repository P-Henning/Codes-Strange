...
public:
  ...
  struct cmp{
    bool operator()(int u,int v){return f[tst][u]>f[tst][v];}
  };
  void astar(state st,int type_st){
    ...
    type[u]=type_st^1,f[tst][u]=h(st);
    priority_queue<int,vector<int>,cmp> q;q.push(u);
    while(!q.empty()){
      u=q.top();q.pop();
      if(u==1)return;
      cur=decode(u);
